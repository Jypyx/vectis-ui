...
Inventaire (exportés depuis `src/index.ts` — l'ordre y fixe la cascade CSS) : **Icon, Button, IconButton, Input, Textarea, Checkbox, Radio, Switch, Tooltip, Dropdown** (+ DropdownItem/DropdownGroup/DropdownSeparator)**, Tabs** (+ TabList/Tab/TabPanel)**, Accordion** (+ AccordionItem)**, Toaster** (+ `toast()`/`dismissToast`)**, Badge, Avatar, Spinner, Combobox, Breadcrumb, Chip, DataTable, InputOTP, Pagination, ProgressLinear, ProgressCircular, Slider**.

Retirés (décision utilisateur — réintroduction possible sur demande) : **Card, Select, Popover, Dialog, DatePicker** (Dialog/DatePicker à refaire plus tard) ; **Alert** remplacé par Toast. La Popover API native reste utilisée par Dropdown/Tooltip/Toast. Tokens orphelins (combobox, calendrier) laissés en place, inoffensifs.
...
- **Combobox** : pattern ARIA combobox/listbox (`aria-activedescendant`, le focus reste dans l'input), multi + tags. Panneau aligné sur le contrôle via `anchor-size(width)`.
...
App de theming temps réel dans `apps/` (manipule `@socle/ui/tokens`, injecte les `--ds-*`, exporte une config) ; playground Nuxt pour valider le SSR de bout en bout ; refonte de Dialog et DatePicker.
