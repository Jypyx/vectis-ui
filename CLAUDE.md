...
**Le périmètre initial + les vagues 5-7 sont terminés** (35 composants + l'API `toast()`/`dismissToast` exportés). Suite naturelle : l'app de theming temps réel dans `apps/` (manipule `@socle/ui/tokens`, injecte les `--ds-*`, exporte une config), et/ou un playground Nuxt pour valider le SSR de bout en bout.
