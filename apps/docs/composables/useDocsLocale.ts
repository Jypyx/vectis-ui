 ...
 * come from a dictionary, the FORMATS from `Intl` on the tag. Only the route SEGMENT is named
 * here; the full BCP 47 tag each segment stands for lives where it is actually consumed — the
 * `TAGS` table in `plugins/vectis.ts` and the `language` field in `nuxt.config.ts`.
 */
