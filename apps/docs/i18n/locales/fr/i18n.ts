export default {
  ...
  frenchBody:
    "<code>fr</code> est optionnel : ne pas l'importer suffit à l'élaguer du bundle. L'argument n'est pas son poids — moins d'un kilo-octet gzippé — mais le fait qu'activer le français livré et ajouter une langue que la bibliothèque ne livre pas soient le MÊME geste, plutôt que deux catégories de dictionnaire.",

  addHeading: 'Ajouter une langue',
  addBody:
    "Un dictionnaire partiel est légitime : ce qui manque retombe sur l'anglais plutôt que sur une clé. La fusion est non récursive par construction — le dictionnaire fait exactement deux niveaux, <code>namespace.key</code>, et c'est ce qui la rend structurellement incapable de descendre dans un message paramétré, qui est une fonction.",
  addTyping:
  ...
  processBody:
    "Le dictionnaire est un état de module, et c'est ce qui permet d'appeler <code>setLocale</code> depuis n'importe quel fichier <code>.ts</code> tout en atteignant des composants déjà montés. La limite acceptée en est l'autre face : le SSR multilingue PAR REQUÊTE n'est pas couvert — un seul processus Node servant deux langues à la fois doit passer les props de texte explicitement.",
  processPrerender:
  ...
  processQuote:
    "Recourez aux props de texte quand un seul processus en cours d'exécution doit répondre en deux langues au même instant. Partout ailleurs — une application dans une langue, ou un site prérendu en plusieurs — <code>setLocale</code> suffit.",
}
