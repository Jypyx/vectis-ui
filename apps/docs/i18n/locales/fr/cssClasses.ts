export default {
  ...
  hiddenBody:
    "Sort un élément de la vue tout en le laissant dans l'arbre d'accessibilité — le libellé dont un lecteur d'écran a besoin et dont un lecteur voyant n'a que faire. Elle vit dans <code>vectis.utilities</code>, la couche la plus forte, et l'emporte donc sur n'importe quelle règle de composant.",
  hiddenNot:
  ...
  propertiesBody:
    "Préférez repointer une propriété à réécrire une règle : <code>--vectis-color-accent</code>, <code>--vectis-radius-interactive</code>, <code>--vectis-icon-size</code>, <code>--vectis-focus-ring-color</code>, <code>--vectis-text-family-heading</code>. Un composant les lit à chaque rendu, si bien que le changement suit chaque état — survol, focus, désactivé — sans qu'aucun d'eux soit redit.",
}
