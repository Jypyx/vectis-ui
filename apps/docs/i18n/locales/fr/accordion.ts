export default {
  ...
  variantsBody:
    "<code>flat</code> ne dessine rien du tout — ni fond, ni bordure, ni rayon — si bien que l'accordéon hérite de la surface sur laquelle il repose. <code>outlined</code> donne la carte canonique : <code>surface-raised</code>, une bordure de 1px, <code>radius-surface</code>, et aucune ombre. C'est la même échelle de décoration qu'emploient VDataTable et VTabs, si bien que les trois s'accordent sans être couplés.",
  variantsRadius:
    "Un coin imbriqué se dérive avec <code>calc(var(--vectis-radius-surface) - 1px)</code>, et c'est pourquoi l'échelle de rayons porte un <code>none</code> qui vaut <code>0px</code> et non <code>0</code> : soustraire d'un zéro sans unité est du CSS invalide.",

  densityHeading: 'Densité',
  densityBody:
    "<code>compact</code> retire 4px à chaque rembourrage — bloc, en ligne et l'air autour du contenu — par un unique delta dont les éléments héritent. La typographie et les icônes ne changent pas : un accordéon n'a pas de hauteur fixe, il est donc entièrement hors de l'échelle de tailles des contrôles.",
