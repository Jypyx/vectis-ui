export default {
  ...
  tonesHeading: 'Tons',
  tonesBody:
    "Ce que l'action SIGNIFIE. Sur un bouton, un ton est une intention, et c'est pourquoi des états comme succès ou avertissement ne sont pas proposés ici. Ceux-là appartiennent aux puces, aux badges et aux notifications.",
  tonesRule:
    "Une règle gouverne les contrôles pleins : ils portent du texte blanc. Tout le reste plie pour que cela reste vrai. Le neutre plein est une inversion complète du texte et de la surface, et son survol mélange vers <code>surface</code> au lieu de s'en éloigner, puisqu'il n'existe pas de neutre plus sombre où aller.",
  ...
  iconsHeading: 'Icônes',
  iconsBody:
    "<code>iconStart</code> et <code>iconEnd</code> prennent un nom d'icône ou un rendu explicite, et les slots <code>#start</code> / <code>#end</code> prennent le relais quand un nom ne suffit pas. VIconButton est le frère carré pour une action sans libellé, et là, la prop <code>label</code> est obligatoire et non optionnelle, puisqu'elle est le seul nom accessible que le contrôle aura jamais.",
