export default {
  ...
  types:
    "Les types sont exportés à côté — <code>VectisMessages</code>, <code>VectisMessagesInput</code>, <code>IconSource</code>, <code>IconResolver</code>, <code>ToastOptions</code> — plus un par composant dont l'API demande à être nommée (<code>ComboboxOption</code>, <code>DataTableColumn</code>, <code>DatePickerSelection</code>…).",

  internalHeading: 'Les fonctions internes, et pourquoi elles le restent',
  internalBody:
    "La bibliothèque porte un jeu complet d'aides pour les dates, les heures, les fichiers et le texte — ce sont elles qui font VDatePicker, VDateInput, VTimePicker et VFilePicker. Elles ne sont PAS exportées, et la raison est énoncée au point d'entrée : les modules internes ne font pas partie de la surface publique, si bien que leurs signatures restent libres de changer avec les composants qui s'en servent.",
  columnModule: 'Module',
