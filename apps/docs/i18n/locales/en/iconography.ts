export default {
  ...
  orderBody:
    'VIcon resolves its source in this order, and the order is the contract: an explicit <code>render</code> → <code>src</code> → <code>name</code> (your resolver, then the built-in registry, then the ligature font) → the slot.',
  noHeuristic:
  ...
  partialBody:
    'A resolver answering <code>undefined</code> means "I do not know this name", not "draw nothing": VIcon then falls back to the registry, and after that to the ligature. That distinction is what makes it worth mapping five names and leaving the rest alone — which is precisely what this site does, since its chrome needs five icons the library has no reason to ship.',
  partialQuote:
