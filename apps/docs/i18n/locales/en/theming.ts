export default {
  ...
  buildHeading: 'Modern CSS and Your Build',
  buildBody:
    'Vectis UI ships the CSS it needs and nothing older: <code>:dir()</code>, <code>color-mix()</code>, OKLCH colors, anchor positioning. Your bundler minifies those sheets along with the rest of your application, and a minifier aimed at browsers below the ones the library supports does not simply drop what they cannot read. It rewrites it.',
  buildDir:
    'One of those rewrites changes what a rule means instead of approximating what it does. Lightning CSS, the default minifier of Vite 8 and of Parcel, replaces <code>:dir(rtl)</code> with a list of <code>:lang()</code> selectors as soon as its targets predate Chrome 120. That list matches on the language of the page, where the library flips on its direction, so the rule never applies at all on an <code>&lt;html dir="rtl" lang="en"&gt;</code> page. Eleven components stop mirroring: the arrows of the pagination, the tabs, the breadcrumb, the menu, the calendar, the date picker and the carousel, the overlay corner of the badge, the direction of travel of the circular progress, the clipped text copy of the linear progress and the wave of the skeleton loader. Nothing throws, and a development server shows none of it, since only a production build minifies.',
  buildFix:
    'Vite derives <code>build.cssTarget</code> from <code>build.target</code>, whose default value stands for browsers far below what the library requires. Naming the supported floor there is the whole fix, and it keeps the output smaller as well: lowered, every OKLCH color of the palette is emitted twice, once as an sRGB fallback and once in another color space.',
}
