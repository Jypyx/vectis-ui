export default {
  ...
  variantsBody:
    '<code>flat</code> draws nothing at all — no background, no border, no radius — so the accordion inherits whatever surface it sits on. <code>outlined</code> gives the canonical card: <code>surface-raised</code>, a 1px border, <code>radius-surface</code>, and no shadow. It is the same decoration scale VDataTable and VTabs use, so the three agree without being coupled.',
  variantsRadius:
    'A nested corner is derived with <code>calc(var(--vectis-radius-surface) - 1px)</code>, which is why the radius scale carries a <code>none</code> that is <code>0px</code> rather than <code>0</code>: subtracting from a unitless zero is invalid CSS.',

  densityHeading: 'Density',
  densityBody:
    "<code>compact</code> takes 4px off every padding — block, inline and the content's breathing room — through a single delta the items inherit. The type and the icons do not change: an accordion has no fixed height, so it is outside the control size scale entirely.",

  animationHeading: 'Animation',
  animationBody:
    'The height animation is progressive enhancement, through <code>::details-content</code> and <code>interpolate-size</code>. Where those are unsupported the section simply opens instantly — no JavaScript measures anything, and nothing jumps.',

  disablingHeading: 'Disabling an item',
  disablingBody:
    '<code>&lt;summary&gt;</code> has no native <code>disabled</code>, so a disabled item gets <code>aria-disabled</code>, <code>tabindex="-1"</code> and a cancelled click — the component\'s only JavaScript. Not <code>pointer-events: none</code>, which would take away the <code>not-allowed</code> cursor and the ability to select the text.',
