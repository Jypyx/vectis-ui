export default {
  ...
  anatomyBody:
    'The label is tied to the field, so clicking it focuses the field, and the hint is read out along with the label. The cross hands focus back to the input the instant it empties the field. Without that, a keyboard reader loses their place in the form.',
  anatomyModel:
  ...
  statesBody:
    '<code>readonly</code> shows the value without allowing changes, and the field can still be focused and copied. That is the difference from <code>disabled</code>, whose value is not even submitted. <code>invalid</code> forces the error state for a rule only the server can check; everything the browser can check, it checks itself.',

  limitsHeading: 'Counters and limits',
  limitsBody:
    '<code>counter</code> shows the length, and with <code>maxlength</code> shows it as a fraction. <code>softLimit</code> turns the limit soft: the reader may type past it, and the field goes into error instead of swallowing the keystroke. That is what you want whenever the text being written is worth more than the rule.',
