export default {
  ...
  colourBody:
    '<code>color</code> REPLACES the tone, and every shade is derived from it with <code>color-mix()</code> — which is what a tag list coming from a database needs, since its colours are data rather than design decisions. Light and dark are handled for you.',

  apiHeading: 'API',
  apiColor: '<code>string</code> — any CSS colour; replaces the tone',
