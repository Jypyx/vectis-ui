export default {
  ...
  types:
    'Types are exported alongside them — <code>VectisMessages</code>, <code>VectisMessagesInput</code>, <code>IconSource</code>, <code>IconResolver</code>, <code>ToastOptions</code> — plus one per component whose API needs naming (<code>ComboboxOption</code>, <code>DataTableColumn</code>, <code>CalendarSelection</code>…).',

  internalHeading: 'The internal helpers, and why they stay internal',
  internalBody:
    'The library carries a full set of date, time, file and text helpers — they are what VCalendar, VDatePicker, VTimePicker and VFileUpload are built on. They are NOT exported, and the reason is stated at the entry point: the internal modules are not part of the public surface, so their signatures stay free to change with the components that use them.',
  columnModule: 'Module',
