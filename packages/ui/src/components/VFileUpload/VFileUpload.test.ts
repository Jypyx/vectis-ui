...
function renderUpload(props: Record<string, unknown> = {}, attrs: Record<string, unknown> = {}) {
  const utils = render(VFileUpload, { props: { title: 'Drop your files', ...props }, attrs })
  const native = utils.container.querySelector<HTMLInputElement>('input[type="file"]')!
  const zone = utils.container.querySelector<HTMLElement>('.v-file-upload-zone')!
...
describe('VFileUpload', () => {
  ...
  it('splits $attrs: the form attributes on the file input, everything else on the zone', () => {
    ...
    expect(container.querySelector('.v-file-upload')!.classList.contains('mine')).toBe(true)
...
describe('VFileUpload preview list', () => {
  ...
  it('is a named list of rows, each with a name, a formatted size and a named remove button', () => {
    ...
    expect(container.querySelector('.v-file-upload-size')!.textContent).toBe('1.2 kB')
  ...
  it('never renders inside the zone, not even when the zone is a button', () => {
    ...
    expect(zone.querySelector('.v-file-upload-list')).toBeNull()
  })
  ...
  it('falls back to the type icon, and honours a typeIcons override', () => {
    ...
    const icons = [...container.querySelectorAll('.v-file-upload-thumb [data-icon]')]
  ...
  it('a removal shrinks the model, emits remove AND change, and hands focus to the next row', async () => {
    ...
    expect(document.activeElement).toBe(
      container.querySelector('.v-file-upload-item .v-file-upload-remove'),
    )
  })
...
describe('VFileUpload thumbnails', () => {
  /*
  ...
  it('renders an <img> for an image and the type icon for anything else', async () => {
    ...
    const [first, second] = container.querySelectorAll('.v-file-upload-thumb')
  ...
  it('falls back to the icon when the image cannot be decoded', async () => {
    ...
    expect(container.querySelector('.v-file-upload-thumb [data-icon]')).toBeTruthy()
    expect(URL.revokeObjectURL).toHaveBeenCalledWith('blob:photo.png')
  })
})

describe('VFileUpload drag & drop', () => {
  const root = (container: Element) => container.querySelector('.v-file-upload')!
...
describe('VFileUpload dev warnings', () => {
  afterEach(() => {
    vi.restoreAllMocks()
  })
