...
export const Loop: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // And round the other way, back to the beginning of the track.
    await userEvent.click(canvas.getByRole('button', { name: 'Next slide' }))
    await waitFor(
      async () => {
        await expect(canvas.getByRole('button', { name: '1 of 6' })).toHaveAttribute(
 ...
 * stops on the last page unless the carousel loops, and `prefers-reduced-motion` disables
 * it outright. A stop control of your own is a binding on the prop — `0` cancels the timer
 * on the spot, and looping makes that binding the only way to stop the movement at all.
 */
