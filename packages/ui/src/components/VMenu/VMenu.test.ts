...
function stubModality(el: Element, keyboard: boolean): () => void {
  ...
  el.matches = function (this: Element, selectors: string) {
    return selectors === ':focus-visible'
      ? keyboard
      : Element.prototype.matches.call(this, selectors)
  }
  return () => {
    if (own) Object.defineProperty(el, 'matches', own)
    else delete (el as Partial<Pick<Element, 'matches'>>).matches
  }
}
...
describe('VMenu', () => {
  ...
  describe('opened with a pointer', () => {
    it('focuses the panel itself, singling out no item', async () => {
      const { getAllByRole, container } = renderMenu()
      ...
      for (const item of getAllByRole('menuitem')) expect(item).not.toBe(document.activeElement)
    })
    ...
    it('Escape closes from the panel and hands the focus back to the trigger', async () => {
      const { getByTestId, container } = renderMenu()
      const menu = await openMenu(container, 'pointer')
    ...
    it('Tab closes the menu from the panel', async () => {
