// @keyboard @core
/**
 * The pure half of VDatePicker's keyboard: it answers the single question "how far
 ...
 *
 * It sits in VDatePicker's own folder rather than in `utils/` because it has exactly
 * one consumer, which fails the admission rule for shared code, and because
 * VDatePicker owns the contract (the same reasoning as `VHotkeys/platform.ts` and
