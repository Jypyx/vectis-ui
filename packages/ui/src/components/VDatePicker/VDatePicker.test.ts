...
describe('VDatePicker', () => {
  ...
  it('selects a date (single selection) and emits an ISO string', async () => {
    ...
    const btn = container.querySelector('.v-date-picker-day[data-selected]') as HTMLElement
  ...
  it('toggles a date in the array (multiple selection)', async () => {
    const { container, emitted, rerender } = render(VDatePicker, {
  ...
  it('disables the dates outside [min,max] and blocks their selection', async () => {
    ...
    const days = [...container.querySelectorAll('.v-date-picker-day')] as HTMLElement[]
  ...
  it('hides the adjacent days when showAdjacentDays is false', () => {
    ...
    const withoutAdjacent = render(VDatePicker, {
      props: { modelValue: JUNE, showAdjacentDays: false },
    })
    const adjacent = (c: HTMLElement) =>
      c.querySelectorAll('.v-date-picker-day--static, .v-date-picker-day[data-outside]').length
  ...
  it('does not display the adjacent days by default', () => {
    ...
    const adjacent = container.querySelectorAll(
      '.v-date-picker-day--static, .v-date-picker-day[data-outside]',
    )
  ...
  it('strikes through the adjacent days outside [min,max]', () => {
    ...
    const struck = container.querySelectorAll('.v-date-picker-day--static[data-disabled]')
  ...
  it('applies the #day slot to the static adjacent days', () => {
    ...
    const statics = [...container.querySelectorAll('.v-date-picker-day--static')]
  ...
  it('renders dots for the events', () => {
    ...
    const selected = container.querySelector('.v-date-picker-day[data-selected]') as HTMLElement
    expect(selected.querySelectorAll('.v-date-picker-dot')).toHaveLength(2)
  })
  ...
  it('opens the months view then selects a month', async () => {
    const { getByRole, getAllByRole } = render(VDatePicker, {
      props: { modelValue: JUNE, locale: 'fr-FR' },
    })
  ...
  it('navigates with the keyboard (arrows) and selects with Enter', async () => {
    const { container, emitted } = render(VDatePicker, { props: { modelValue: JUNE } })
