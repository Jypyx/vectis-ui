 ...
 * DropdownMenu ouvert par un bouton « … ».
 */
      ...
      <template v-for="(item, index) in visibleItems" :key="item.href">
        ...
        <li v-if="truncated && index === 1" class="ds-breadcrumb-item ds-breadcrumb-ellipsis">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuItem v-for="hidden in hiddenItems" :key="hidden.href" :href="hidden.href">
              <template v-if="hidden.iconStart" #icon>
                <Icon v-bind="iconProps(hidden.iconStart)" />
              </template>
              {{ hidden.label }}
            </DropdownMenuItem>
          </DropdownMenu>
