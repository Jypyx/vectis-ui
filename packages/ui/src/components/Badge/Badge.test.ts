...
describe('Badge', () => {
  it('standalone par défaut : racine .ds-badge seule, tone accent, count rendu', () => {
    const { container, getByText } = render(Badge, { props: { count: 3 } })
    const root = container.firstElementChild!
    expect(root.classList.contains('ds-badge')).toBe(true)
    expect(container.querySelector('.ds-badge-host')).toBeNull()
  ...
  it('plafonnement : count > 99 affiché « 99+ », 99 inchangé', () => {
    ...
    expect(container.querySelector('.ds-badge')!.textContent).toBe('99+')

    const { container: at99 } = render(Badge, { props: { count: 99 } })
    expect(at99.querySelector('.ds-badge')!.textContent).toBe('99')
  })

  it('icon : data-icon-only, .ds-icon rendu, prime sur count', () => {
    const { container } = render(Badge, { props: { icon: 'notifications', count: 3 } })
    const badge = container.querySelector('.ds-badge')!
    expect(badge.hasAttribute('data-icon-only')).toBe(true)
    expect(badge.querySelector('.ds-icon')).toBeTruthy()
  ...
  it('dot : data-dot, aucun contenu, icon et count ignorés', () => {
    ...
    expect(badge.querySelector('.ds-icon')).toBeNull()
    expect(badge.textContent).toBe('')
  })

  it('slot défaut : racine .ds-badge-host, cible rendue, badge en dernier enfant', () => {
    const { container, getByText } = render(Badge, {
      props: { count: 3 },
      slots: { default: '<button type="button">Messages</button>' },
    })
    const host = container.firstElementChild!
    expect(host.classList.contains('ds-badge-host')).toBe(true)
    expect(host.hasAttribute('data-overlay')).toBe(false)
    expect(getByText('Messages')).toBeTruthy()
    expect(host.lastElementChild!.classList.contains('ds-badge')).toBe(true)
  })
  ...
  it('overlay : data-overlay posé sur le host', () => {
    ...
    expect(container.querySelector('.ds-badge-host')!.hasAttribute('data-overlay')).toBe(true)
  })

  it('bordered : data-bordered posé sur le badge', () => {
    const { container } = render(Badge, { props: { count: 3, bordered: true } })
    expect(container.querySelector('.ds-badge')!.hasAttribute('data-bordered')).toBe(true)
  })

  it('fallthrough : class sur .ds-badge en standalone, sur .ds-badge-host avec cible', () => {
    const { container } = render(Badge, {
      props: { count: 3 },
    ...
    expect(hosted.querySelector('.ds-badge-host')!.classList.contains('extra')).toBe(true)
    expect(hosted.querySelector('.ds-badge')!.classList.contains('extra')).toBe(false)
  })
})
