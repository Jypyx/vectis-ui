...
export default tseslint.config(
  ...
  {
    /*
     * The documentation site writes no inline style, and this is what holds it to that.
     *
     * It used to write a great many: the sheets were declared `LAYOUT ONLY` and everything else
     * was an attribute on the element, so that it painted as the markup streamed. That argument
     * died when `apps/docs/nuxt.config.ts` set `features.inlineStyles: false` — the sheets ship
     * as render-blocking `<link>`s, so a class is ready at the same instant and is written once
     * instead of on every node of 106 prerendered pages, in two languages.
     *
     * Scoped to the site on purpose. The LIBRARY legitimately writes `:style` (a fill fraction,
     * a dial angle, a custom tone colour — values a stylesheet cannot know), and its stories lay
     * themselves out inline; `allowBinding` would cover the first but not the second, and
     * neither is what this rule is here to protect.
     */
    files: ['apps/docs/**/*.vue'],
    rules: {
